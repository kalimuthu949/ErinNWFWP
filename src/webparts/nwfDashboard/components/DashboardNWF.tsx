...
export default function NWDashBoardAdmin(
    ...
    <div style={{ margin: "1rem 2rem" }}>
      ...
      (<div className="DetailsList">
        <Pagination
          ...
          totalPages={
            masterRows.length > 0 ? Math.ceil(masterRows.length / totalPage) : 1
          }
          onChange={(page) => {
            paginate(page);
          }}
      ...
      :<div style={{fontWeight:"bold",textAlign:"center"}}>No Data Found</div>
      }
      <Dialog
        hidden={hideDialog}
      ...
      >
        ...
        >
          ...
          />
          <PoepleSharedUsers peoples={users} update={updateSharedWithID} GetUserDetails={GetsharedwithUserDetails}/>
          {/*<PeoplePicker
    ...
    if (selectedItem[0]["SharedWith"]) {
      for (var i = 0; i < selectedItem[0]["SharedWith"].length; i++) {
        ...
        getSharedwithusers.push({
          imageUrl:
            "/_layouts/15/userphoto.aspx?size=S&accountname="+selectedItem[0]["SharedWith"][i].EMail,
  ...
  function doesTextStartWith(text: string, filterText: string): boolean {
    ...
    {
      await INWfDashboardProps.spcontext.siteUsers.getByEmail(UserEmail).groups.get().then(async function(data)
      {
        //console.log(data);
      }).catch(function(error){
        //alert('error');
        ...
        await INWfDashboardProps.spcontext.siteUsers.getByEmail(UserEmail).groups.get().then(async function(data)
        {
           if(data.length>0)
           {
                ...
                await groups.forEach(async groupid => 
                        {
                            await INWfDashboardProps.spcontext.siteGroups.getById(groupid).users.get().then(async function(result) 
                                {
